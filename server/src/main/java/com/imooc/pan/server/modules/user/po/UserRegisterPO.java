...
public class UserRegisterPO implements Serializable {
    ...
    private static final long serialVersionUID = -5521427831609988931L;

    @ApiModelProperty(value = "用户名" , required = true)
    @NotBlank(message = "用户名不能为空")
    @Pattern(regexp = "^[0-9A-Za-z]{6,16}$",message = "请输入6-16位只包含你数字和字母的用户名")
    private String username;

    @ApiModelProperty(value = "密码" , required = true)
    @NotBlank(message = "密码不能为空")
    @Length(min = 8,max = 16,message = "输入8-16位密码")
    private String password;

    @ApiModelProperty(value = "密码问题" , required = true)
    @NotBlank(message = "密码问题不能为空")
    @Length(max = 100,message = "密保问题不能超过100个字符")
    private String question;

    @ApiModelProperty(value = "密码答案" , required = true)
    @NotBlank(message = "答案不能为空")
    @Length(max = 100,message = "答案不能超过100个字符")
